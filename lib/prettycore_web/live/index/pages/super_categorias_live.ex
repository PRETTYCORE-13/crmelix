defmodule PrettycoreWeb.SuperCategoriasLive do
  ...
  def mount(_params, _session, socket) do
    socket =
      socket
      |> assign(:current_page, "super_categorias")
      |> assign(:sidebar_open, true)
      |> assign(:show_programacion_children, false)
     |> assign(:show_clientes_children, false)
     |> assign(:show_prettycore_children, false)
      |> assign(:super_categorias, [])
      |> assign(:modal, nil)
      |> assign(:selected, nil)
      |> assign(:form_nombre, "")
      ...
      |> assign(:sc_productos_codigos, MapSet.new())
      |> assign(:modal_search, "")
      |> allow_upload(:imagen,
  ...
  @impl true
  def handle_info(:load_super_categorias, socket) do
    {:noreply, assign(socket, super_categorias: SuperCategorias.list_super_categorias())}
  end
  ...
  def handle_event("nueva", _, socket) do
    {:noreply, assign(socket,
      ...
      form_nombre: "",
      form_descripcion: "",
      form_orden: "#{length(socket.assigns.super_categorias)}",
      form_error: nil
    )}
  end

  def handle_event("editar", %{"id" => id}, socket) do
    sc = SuperCategorias.get_super_categoria_con_productos(id)
    todos = Productos.list_productos()
    codigos = sc.productos |> Enum.map(& &1.codigo) |> MapSet.new()
    {:noreply, assign(socket,
      modal: :editar,
      selected: sc,
      form_nombre: sc.nombre,
      form_descripcion: sc.descripcion || "",
      form_orden: "#{sc.orden}",
      form_error: nil,
      todos_productos: todos,
      sc_productos_codigos: codigos,
      modal_search: ""
    )}
  end
  ...
  def handle_event("edit_imagen", %{"id" => id}, socket) do
    ...
    {:noreply, assign(socket, modal: :imagen, selected: sc, upload_error: nil, uploading: false)}
  end
  ...
  def handle_event("toggle_producto", %{"codigo" => codigo}, socket) do
    codigos = socket.assigns.sc_productos_codigos
    codigos =
      if MapSet.member?(codigos, codigo),
        do: MapSet.delete(codigos, codigo),
        else: MapSet.put(codigos, codigo)
    {:noreply, assign(socket, sc_productos_codigos: codigos)}
  end
  ...
  def handle_event("subir_imagen", _params, socket) do
    ...
    if entries == [] do
      ...
      case results do
        [{:ok, url}] ->
          {:ok, updated} = SuperCategorias.update_super_categoria(sc, %{imagen_url: url})
          scs = SuperCategorias.list_super_categorias()
          {:noreply,
           socket
           |> assign(super_categorias: scs, selected: updated, uploading: false, upload_error: nil, modal: nil)
           |> put_flash(:info, "✓ Imagen de \"#{sc.nombre}\" actualizada correctamente")}
        [{:error, reason}] ->
    ...
    <div class="flex min-h-screen bg-gray-100">
      ...
      <header class="mb-6">
        <div class="flex items-center justify-between gap-4">
          <div>
            <h1 class="text-2xl font-bold text-gray-900">Super Categorías</h1>
            <p class="text-sm text-gray-500 mt-0.5">Agrupa productos por familias, colecciones o cualquier criterio</p>
          </div>
          <button
            phx-click="nueva"
      ...
      <!-- Grid de super categorías -->
      <%= if Enum.empty?(@super_categorias) do %>
        <div class="flex flex-col items-center justify-center py-24 text-gray-400">
          ...
          <p class="text-sm font-medium">Sin super categorías todavía</p>
          <p class="text-xs mt-1">Crea una para agrupar productos por familia o colección</p>
        </div>
      <% else %>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:border-purple-200 transition-all group flex flex-col">
              ...
              <div class="relative w-full aspect-square bg-gray-50 flex items-center justify-center overflow-hidden">
                <%= if sc.imagen_url && sc.imagen_url != "" do %>
                  <img
                    src={"#{sc.imagen_url}?t=#{DateTime.to_unix(sc.updated_at)}"}
                    alt={sc.nombre}
              ...
              <div class="p-3 flex flex-col flex-1">
                ...
                <div class="mt-auto pt-2 flex gap-1.5">
                  ...
                  <button
                    phx-click="eliminar"
                    phx-value-id={sc.id}
                    data-confirm={"¿Eliminar '#{sc.nombre}'?"}
          ...
          <div class={"relative bg-white rounded-2xl shadow-2xl w-full z-10 flex flex-col #{if @modal == :editar, do: "max-w-2xl max-h-[90vh]", else: "max-w-sm"}"}>
            <div class="p-6 pb-0">
              <h2 class="text-lg font-bold text-gray-900 mb-4">
                <%= if @modal == :nueva, do: "Nueva super categoría", else: "Editar super categoría" %>
              </h2>
            </div>
      ...
      <%= if @modal == :imagen do %>
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 z-10">
            <h2 class="text-lg font-bold text-gray-900 mb-1">Imagen de super categoría</h2>
