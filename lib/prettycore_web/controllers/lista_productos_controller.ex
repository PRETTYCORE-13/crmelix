defmodule PrettycoreWeb.ListaProductosController do
  use PrettycoreWeb, :controller
  alias Prettycore.Catalogos.Listas.ListasProductosContext
  ...
  def index(conn, params) do
    ...
    {total, data} =
      if cliente_id do
        {ListasProductosContext.count_listas_por_cliente(cliente_id),
         ListasProductosContext.listar_listas_por_cliente(cliente_id, offset, limit)}
      else
        {ListasProductosContext.count_listas(),
         ListasProductosContext.listar_listas(offset, limit)}
      end

    json(conn, %{ok: true, tabla: "listas_productos", total: total, page: page,
  ...
  def create(conn, params) do
    case ListasProductosContext.crear_lista(params) do
  ...
  def update(conn, %{"id" => id} = params) do
    case ListasProductosContext.buscar_lista(id) do
      {:error, :not_found} ->
        conn |> put_status(404) |> json(%{ok: false, error: "Lista no encontrada"})
      {:ok, lista} ->
        case ListasProductosContext.actualizar_lista(lista, params) do
  ...
  def agregar_producto(conn, %{"lista_id" => lista_id, "producto_id" => producto_id}) do
    case ListasProductosContext.agregar_producto(lista_id, producto_id) do
  ...
  def quitar_producto(conn, %{"lista_id" => lista_id, "producto_id" => producto_id}) do
    case ListasProductosContext.quitar_producto(lista_id, producto_id) do
