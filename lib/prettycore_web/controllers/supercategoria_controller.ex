defmodule PrettycoreWeb.SupercategoriaController do
  use PrettycoreWeb, :controller
  alias Prettycore.Catalogos.Supercategorias.SupercategoriasContext

  def index(conn, params) do
    page   = params |> Map.get("page",  "1")  |> parse_int(1)
    limit  = params |> Map.get("limit", "50") |> parse_int(50)
    offset = (page - 1) * limit
    total  = SupercategoriasContext.count_supercategorias()
    data   = SupercategoriasContext.listar_supercategorias(offset, limit)

    json(conn, %{ok: true, tabla: "supercategorias", total: total, page: page,
                 limit: limit, paginas: ceil(total / limit), data: data})
  end

  def buscar_id(conn, %{"id" => id}) do
    case SupercategoriasContext.buscar_supercategoria(id) do
      {:ok, s}             -> json(conn, %{ok: true, data: s})
      {:error, :not_found} -> conn |> put_status(404) |> json(%{ok: false, error: "Supercategoría no encontrada"})
    end
  end

  def create(conn, params) do
    case SupercategoriasContext.crear_supercategoria(params) do
      {:ok, s}            -> conn |> put_status(201) |> json(%{ok: true, data: s})
      {:error, changeset} -> json(conn, %{ok: false, errors: format_errors(changeset)})
    end
  end

  def update(conn, %{"id" => id} = params) do
    case SupercategoriasContext.buscar_supercategoria(id) do
      {:error, :not_found} ->
        conn |> put_status(404) |> json(%{ok: false, error: "Supercategoría no encontrada"})
      {:ok, s} ->
        case SupercategoriasContext.actualizar_supercategoria(s, params) do
          {:ok, actualizada}  -> json(conn, %{ok: true, data: actualizada})
          {:error, changeset} -> conn |> put_status(422) |> json(%{ok: false, errors: format_errors(changeset)})
        end
    end
  end

  defp format_errors(changeset) do
    Ecto.Changeset.traverse_errors(changeset, fn {msg, opts} ->
      Enum.reduce(opts, msg, fn {k, v}, acc -> String.replace(acc, "%{#{k}}", to_string(v)) end)
    end)
  end

  defp parse_int(val, default) do
    case Integer.parse(to_string(val)) do
      {n, _} when n > 0 -> n
      _                  -> default
    end
  end
end
