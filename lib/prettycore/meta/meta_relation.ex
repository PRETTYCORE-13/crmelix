defmodule Prettycore.Meta.MetaRelation do
  ...
  schema "meta_relations" do
    ...
    field :modelo_destino_id, :binary_id
    field :tipo,              :string
    field :campo_fk,          :string
    field :alias,             :string

    belongs_to :modelo,   Prettycore.Meta.MetaModel, foreign_key: :meta_model_id,     define_field: false
    belongs_to :destino,  Prettycore.Meta.MetaModel, foreign_key: :modelo_destino_id, define_field: false
  ...
  def changeset(rel, attrs) do
    rel
    |> cast(attrs, [:meta_model_id, :modelo_destino_id, :tipo, :campo_fk, :alias])
    |> validate_required([:meta_model_id, :modelo_destino_id, :tipo, :campo_fk])
